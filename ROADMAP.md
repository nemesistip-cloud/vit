...
| 15 | **Training Pipeline (In-App)** | `app/api/routes/training.py` | ✅ **Partial fix (Apr 2026)**: In-app trigger now actually trains all 12 models with differentiated learned state and reports real per-model metrics (accuracy, log-loss, Brier, over/under) computed by `_evaluate_model_on_history()`. ✅ **Apr 22 2026 — Versioned promotion**: `POST /api/ai-engine/upload/{key}` now stages each `.pkl` as `{key}__{version}.pkl`, refuses to overwrite an existing version (HTTP 409), and appends to `model_metadata.version_history`. Active production only changes when `auto_promote=true` is passed or operator calls new endpoint `POST /api/ai-engine/models/{key}/promote/{version}`. New `GET /api/ai-engine/models/{key}/versions` lists history. Previous artifacts are never deleted. Remaining gap: training status is still stored in-memory only — wiped on restart. No persistent job tracking yet. The referenced `colab/train_real_match_models.py` script does not exist; use `scripts/train_models.py` for offline training. |
| 15a | **Real-data-only fixtures + dedup** | `app/api/routes/matches.py`, `app/api/routes/admin.py`, `app/data/match_dedup.py` | ✅ **New (Apr 22 2026)**: Synthetic-fixture fabrication is now disabled by default — gated behind `ENABLE_SYNTHETIC_FIXTURES=true`. When `FOOTBALL_DATA_API_KEY` is missing or rate-limited, sync routes return a clean message instead of inventing matches. Every `Match` row now carries `source` (`manual_upload` / `footballdata` / `odds_api` / `seed` / `synthetic` / etc.) and a deterministic `fingerprint` (`YYYY-MM-DD::home::away::league`, normalised). API fixture sync dedups against both `external_id` and fingerprint, so manually-uploaded matches are never overwritten by an API hit. Migration `005_match_source_and_model_versions.py` adds the columns; startup auto-backfills fingerprints on legacy rows (136 matches backfilled on first boot). |
| 15b | **FT results backfill + final-time guarantee** | `app/services/ft_backfill.py`, `app/api/routes/admin.py`, `frontend/src/pages/admin.tsx` | ✅ **New (Apr 22 2026)**: New `app/services/ft_backfill.py` makes sure every past match has a final score. It runs `settle_results()` for real-source matches (footballdata/odds_api) and **deterministically simulates** scores for past local-only/seed/synthetic matches (Poisson-ish total-goals distribution centred on per-league averages, outcome split via implied 1X2 probability with `fingerprint` as RNG seed → re-runs always produce the same result). Simulated matches are tagged `source='<orig>+sim_ft'`. New endpoint `POST /admin/matches/backfill-ft-results?settle_real=...&simulate_local=...&days_back=...`. **Verified**: 38 past matches that were stuck on `scheduled`/`upcoming`/`live` have been settled — `0` past unfinished matches remain in DB. |
| 15c | **Football-Data.org admin integration polish** | `app/api/routes/admin.py`, `frontend/src/pages/admin.tsx` | ✅ **New (Apr 22 2026)**: (1) Admin can paste/update `FOOTBALL_DATA_API_KEY` via the existing API Keys panel — change applies immediately and now also resets the in-memory invalidation flags (`_KEY_PERMANENTLY_INVALID`, `_FORBIDDEN_LEAGUES`) so the new key gets a fresh chance without restart. (2) New endpoint `POST /admin/data-sources/test/{key_name}` (supports `football_data` and `odds_api`) for one-click connection testing with detailed status (ok/no_key/invalid/forbidden/rate_limited/down). (3) Admin frontend (`SystemTab` → new `FootballDataCard`) gets four buttons: **Test Connection**, **Fetch Upcoming Fixtures**, **Sync FT Results**, **Backfill Past Results** — all wired to live mutations with toast feedback. |

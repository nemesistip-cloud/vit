...
# Allow overriding league-average odds via external JSON config
_OVERRIDDEN_LEAGUE_ODDS = {}
try:
    import json, os
    _cfg_path = os.path.join(os.path.dirname(__file__), '..', 'config', 'league_average_odds.json')
    if os.path.exists(_cfg_path):
        with open(_cfg_path, 'r') as _f:
            _OVERRIDDEN_LEAGUE_ODDS = json.load(_f)
except Exception:
    _OVERRIDDEN_LEAGUE_ODDS = {}
...
class MarketUtils:
    ...
    def get_fallback_odds(league: str = "default") -> Dict[str, float]:
        ...
        """
        # Prefer overrides if provided via JSON config
        odds_source = _OVERRIDDEN_LEAGUE_ODDS.get(league) if isinstance(_OVERRIDDEN_LEAGUE_ODDS, dict) else None
        if odds_source:
            odds = dict(odds_source)
        else:

...
export function PredictionFlow({ match, open, onClose }: PredictionFlowProps) {
  ...
  const screenshotGroups = ["double_chance", "over_under_1.5", "over_under_3.5", "draw_no_bet", "asian_handicap", "btts_half_time", "home_clean_sheet", "away_clean_sheet"];

  marketGroups.push({ id: "1x2", title: KNOWN_MARKETS["1x2"].title, sides: KNOWN_MARKETS["1x2"].sides });

  for (const gid of screenshotGroups) {
    marketGroups.push({ id: gid, title: KNOWN_MARKETS[gid].title, sides: KNOWN_MARKETS[gid].sides });
  }

...
import {
  Brain, Send, User as UserIcon,
  RotateCw, ChevronRight, ChevronDown
} from "lucide-react";
...
function ThoughtsToggle({ thoughts }: { thoughts: string[] }) {
  const [open, setOpen] = useState(false);
  return (
    <div className="mb-1.5">
      <button
        onClick={() => setOpen(o => !o)}
        className="flex items-center gap-1 text-[9px] font-mono text-vit-green/50 hover:text-vit-green/80 transition-colors"
      >
        {open ? <ChevronDown size={10} /> : <ChevronRight size={10} />}
        {open ? "hide reasoning" : "show reasoning"}
      </button>
      {open && (
        <div className="mt-1 p-2 rounded-lg bg-vit-void border border-vit-border text-[9px] font-mono text-vit-green/60 space-y-0.5">
          {thoughts.map((t, i) => (
            <div key={i}>[NODE] {t}</div>
          ))}
        </div>
      )}
    </div>
...
export default function AssistantPage() {
  ...
  async function send(text: string) {
    ...
    try {
      ...
      const content = response.reply ?? response.content ?? "";
      if (!content) {
        toast.error("No response received from VIT Brain.");
        return;
      }
              ...
              <div className="grid grid-cols-1 gap-2 w-full max-w-sm">
                {SUGGESTED_PROMPTS.map(p => (
                  <button
                    key={p}
                    onClick={() => send(p)}
                    className="p-3 rounded-xl bg-vit-surface-2 border border-vit-border text-left hover:border-vit-green/30 transition-all group"
                  >
          ...
          {messages.map((m, i) => (
            <div key={i} className={`flex gap-3 ${m.role === 'user' ? 'flex-row-reverse' : ''}`}>
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 border ${
                m.role === 'user'
                  ? 'bg-vit-surface-3 border-vit-border text-vit-text-2'
                  : 'bg-vit-green-glow border-vit-green/20 text-vit-green'
              }`}>
              ...
              <div className={`max-w-[85%] space-y-1 ${m.role === 'user' ? 'items-end' : 'items-start'} flex flex-col`}>
                ...
                {m.role === 'assistant' && m.thoughts && m.thoughts.length > 0 && (
                  <ThoughtsToggle thoughts={m.thoughts} />
                )}
                ...
                {m.content ? (
                  <div className={`p-4 rounded-2xl text-sm leading-relaxed ${
                    m.role === 'user'
                      ? 'bg-vit-surface-3 text-vit-text-1'
                      : 'bg-vit-surface-2 border border-vit-border text-vit-text-2'
                  }`}>
                    {m.role === 'user'
                      ? m.content

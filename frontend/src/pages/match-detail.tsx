      ...
      {/* ── Header ── */}
        <div className="sticky top-0 z-40 bg-[#0C0E12]/80 backdrop-blur-md border-b border-white/5 p-4 flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-mono text-[9px] text-primary uppercase tracking-[0.2em]">{match?.league || 'UNKNOWN LEAGUE'}</p>
        ...
        <div className="max-w-4xl mx-auto p-4 space-y-6">
          ...
          <div className="relative overflow-hidden rounded-sm border border-white/5 bg-gradient-to-br from-white/[0.03] to-transparent p-8 text-center">
             ...
             <div className="flex items-center justify-between gap-4 relative z-10">
                <div className="flex-1 space-y-4">
                   ...
                   <p className="text-sm font-display font-bold uppercase tracking-widest">{match?.home_team || 'HOME'}</p>
                </div>
                ...
                <div className="flex-1 space-y-4">
                   ...
                   <p className="text-sm font-display font-bold uppercase tracking-widest">{match?.away_team || 'AWAY'}</p>
                </div>
             </div>
             ...
             <div className="mt-10 flex flex-wrap justify-center gap-6 border-t border-white/5 pt-6">
                <div className="flex items-center gap-2 text-muted-foreground">
                   ...
                   <span className="font-mono text-[10px] uppercase tracking-wider">{match?.kickoff_time ? safeFormat(match.kickoff_time, 'MMM dd, yyyy • HH:mm') : 'PENDING'}</span>
                </div>
                <div className="flex items-center gap-2 text-muted-foreground">
                   <MapPin size={14} />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
             <div className="lg:col-span-8 space-y-6">
                <ConsensusPanel data={consensus} homeTeam={match?.home_team} awayTeam={match?.away_team} />
                ...
                <MarketEdgeTable odds={match?.odds} match={match} edge={marketEdge} />
             </div>
                   ...
                   <div className="space-y-4">
                      <div>
                         ...
                         <p className="text-lg font-bold text-foreground">
                            {Number(consensus?.home_prob || 0) > Number(consensus?.away_prob || 0) ? (match?.home_team || 'Home') : (match?.away_team || 'Away')} Win
                         </p>
                      </div>
                      <div className="grid grid-cols-2 gap-4">
                         <div>
                            <p className="text-[9px] text-muted-foreground uppercase font-mono mb-1">Edge</p>
                            <p className="text-sm font-bold text-vit-positive">{(Number(marketEdge?.edge || 0) * 100).toFixed(1)}%</p>
                         </div>
                         <div>
                            <p className="text-[9px] text-muted-foreground uppercase font-mono mb-1">Confidence</p>
                            <p className="text-sm font-bold">{(Number(consensus?.confidence || 0) * 100).toFixed(0)}%</p>
                         </div>
                      </div>
                      <div className="pt-2">
                         <Badge className="w-full justify-center bg-white/5 border-white/10 text-[10px] font-mono py-1 rounded-sm">
                            INVESTMENT GRADE: {Number(marketEdge?.edge || 0) > 0.04 ? 'AAA' : 'BBB+'}
                         </Badge>
                      </div>
                   </div>
                </div>
                ...
                <Card className="border-white/5 bg-white/[0.01]">
                   ...
                   <CardContent className="space-y-4 pt-4">
                      <div className="flex justify-between items-center">
                         ...
                         <span className={`text-[10px] font-mono font-bold ${Number(consensus?.elo_diff || 0) > 0 ? 'text-vit-positive' : 'text-secondary'}`}>
                            {Number(consensus?.elo_diff || 0) > 0 ? '+' : ''}{Number(consensus?.elo_diff || 0).toFixed(0)}
                         </span>
                      </div>
                      <div className="flex justify-between items-center">
                         <div className="flex items-center gap-2">
                         ...
                         <span className="text-[10px] font-mono font-bold text-foreground">
                            {Number(consensus?.squad_value_diff || 0) > 0 ? '+' : ''}{Number(consensus?.squad_value_diff || 0)}%
                         </span>
                      </div>
                      <div className="flex justify-between items-center">
                         <div className="flex items-center gap-2">
                         ...
                         <span className="text-[10px] font-mono font-bold">{Number(consensus?.risk_score || 0).toFixed(1)}/10</span>
                      </div>
                      <div className="flex justify-between items-center">
                         <div className="flex items-center gap-2">
                         ...
                         <span className="text-[10px] font-mono font-bold text-vit-positive">{(Number(consensus?.model_agreement || 0) * 100).toFixed(0)}%</span>
                      </div>
                   </CardContent>
                </Card>
           ...
           <div className="max-w-4xl mx-auto flex items-center justify-between gap-6">
              <div className="hidden sm:block">
                 ...
                 <div className="flex gap-4 mt-1">
                    <span className="text-xs font-mono font-bold text-primary">EDGE: {(Number(marketEdge?.edge || 0) * 100).toFixed(1)}%</span>
                    <span className="text-xs font-mono font-bold text-vit-positive">ROI: {(Number(marketEdge?.expected_roi || 0)).toFixed(1)}%</span>
                 </div>
              </div>
              <Button

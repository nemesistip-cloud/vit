...
def sha3_256_hex(data: bytes) -> str:
    """Returns lowercase hex SHA3-256 digest for state commitments."""
...
def hash_block_header(prev_hash: str, merkle_root: str,
                    ...
                    validator_id: str, version: int = 1, nonce: int = 0,
                    state_commitment: str = "") -> str:
    """Canonical block header hash — deterministic field ordering."""
    header_str = f"{version}:{prev_hash}:{merkle_root}:{timestamp}:{height}:{validator_id}:{nonce}:{state_commitment}"

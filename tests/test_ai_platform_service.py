...
def test_platform_assistant_routes_expose_shared_gateway_api():
    from types import SimpleNamespace

    from fastapi import FastAPI
    from fastapi.testclient import TestClient

    from app.api.routes.ai_assistant import router
    from app.auth.dependencies import get_current_user

    app = FastAPI()
    app.include_router(router, prefix="/api")
    app.dependency_overrides[get_current_user] = lambda: SimpleNamespace(id=42, role="admin")
    client = TestClient(app)

    status_response = client.get("/api/ai/assistant/platform/status")
    assert status_response.status_code == 200
    status_payload = status_response.json()
    assert status_payload["available"] is True
    assert status_payload["gateway_route"] == "/api/ai/assistant/platform/chat"

    chat_response = client.post(
        "/api/ai/assistant/platform/chat",
        json={"message": "open ai", "session_id": "route-session", "workspace_id": "route-workspace", "execute": True},
    )
    assert chat_response.status_code == 200
    payload = chat_response.json()
    assert payload["provider"] == "platform"
    assert payload["response"]["action"] == "execute_command"
    assert payload["response"]["command"] == "open_ai"
    assert len(payload["history"]) == 2

    history_response = client.get(
        "/api/ai/assistant/platform/history",
        params={"session_id": "route-session", "workspace_id": "route-workspace"},
    )

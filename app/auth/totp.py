...
SEC-09: TOTP columns are defined in app/db/models.py (User.totp_secret,
User.totp_secret_pending, User.totp_enabled). Runtime ALTER TABLE DDL has been
removed — columns are created via the normal schema-creation path in start.sh.
...
async def setup_2fa(
    ...
    Stores the pending secret on the User model (column added via Alembic migration).
    Does NOT enable 2FA — the user must call /enable after scanning.
    """
    ...
    return {
        ...
        "instructions": "Scan the QR code with Google Authenticator or Authy, then call /auth/2fa/enable with a TOTP code to confirm.",
    }
